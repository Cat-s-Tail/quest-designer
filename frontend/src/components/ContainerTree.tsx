...
export default function ContainerTree({ containers, selectedContainer, onSelectContainer, onAddContainer: _onAddContainer, onRelink, onUpdatePosition, onGetAllPositions }: any) {
  ...
  const initialNodes = useMemo(() => {
    return containers.map((container: any) => {
      ...
      const sectionCount = container.sections?.length || 0
      ...
      return {
        ...
        data: {
          label: (
            <div className="text-center p-2">
              <div className="font-bold text-sm mb-1">{container.name}</div>
              <div className="text-xs text-slate-300">
                {container.maxWeight > 0 ? `Max: ${container.maxWeight}kg` : 'No weight limit'}
              </div>
        ...
        style: {
          background: isSelected ? '#c026d3' : '#334155',
          color: '#fff',
          border: isSelected ? '3px solid #e879f9' : '2px solid #64748b',
          borderRadius: '8px',
          padding: '4px',
          cursor: 'pointer',
          minWidth: '180px',
  ...
  const initialEdges = useMemo(() => {
    ...
    containers.forEach((container: any) => {
      if (container.next && Array.isArray(container.next)) {
        container.next.forEach((nextId: string) => {
          edges.push({
            id: `${container.id}->${nextId}`,
            source: container.id,
            target: nextId,
            animated: true,
            ...
            labelBgStyle: { fill: '#86198f', fillOpacity: 0.8 },
            labelBgPadding: [4, 4],
  ...
  }, [layoutedNodes, layoutedEdges, setNodes, setEdges, containersVersion])

  const onConnect = useCallback(
    (params: any) => {
      setEdges((eds) => addEdge({ ...params, animated: true, style: { stroke: '#e879f9', strokeWidth: 2 } }, eds))
      if (onRelink) {
